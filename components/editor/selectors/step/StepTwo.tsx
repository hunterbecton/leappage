...
import { StepTwoProps } from './_models';
...
export const StepTwo = (props: Partial<StepTwoProps>) => {
  props = {
    ...defaultProps,
    ...props,
  };
      ...
      <div className='mx-auto flex max-w-7xl flex-wrap'>
        {steps.map((step, i) => (
          ...
          >
            ...
            <div className='title-font bg-primary relative z-10 mt-10 inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-sm font-medium text-white sm:mt-0'>
              {i + 1}
            </div>
            <div className='flex flex-grow flex-col items-start pl-6 sm:flex-row sm:items-center md:pl-8'>
              <div className='bg-primary-light text-primary inline-flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full'>
                <Icon id={step.icon.id} renderStyle='w-8 h-8 text-primary' />
              </div>
