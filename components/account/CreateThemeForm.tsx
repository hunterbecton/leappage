...
import { CreateThemeData } from './_models';

const validationSchema: SchemaOf<CreateThemeData> = object().shape({
  primary: string()
...
export const CreateThemeForm: FC = () => {
  const [isSending, setIsSending] = useState(false);

  const { user } = useAuth();

  const methods = useForm<CreateThemeData>({
  ...
  const handleCreateTheme = async (newData: CreateThemeData) => {
    let isValid = false;
      ...
      <div className='space-y-6 lg:col-span-9 lg:px-0'>
        {restrict(['admin'], user) && (
          <FormProvider {...methods}>
            <form onSubmit={methods.handleSubmit(handleCreateTheme)}>
